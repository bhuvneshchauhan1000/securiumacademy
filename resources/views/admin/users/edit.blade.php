<x-app-layout>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('users.update', \App\Support\HashId::encode($user->id)) }}">
                @csrf
                @method('PUT')
                ...
                <div class="overflow-hidden bg-white dark:bg-gray-800 shadow-sm sm:rounded-xl">
                    <div class="bg-gradient-to-r from-indigo-600 via-purple-600 to-sky-600 px-6 py-6 sm:px-8">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-white">{{ __('Edit User') }}</h3>
                                <p class="text-sm text-indigo-100">{{ __('Update the details for :name.', ['name' => $user->name]) }}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="border-t border-gray-100 pt-8 dark:border-gray-700">
                            ...
                            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <x-text-input id="password" class="mt-1 block w-full" type="password" name="password" autocomplete="new-password" placeholder="••••••••" />
                                    <x-input-error :messages="$errors->get('password')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="password_confirmation" :value="__('Confirm New Password')" />
                                    <x-text-input id="password_confirmation" class="mt-1 block w-full" type="password" name="password_confirmation" autocomplete="new-password" placeholder="••••••••" />
                                ...
                                <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                    @forelse ($roles as $role)
                                        <label class="relative flex cursor-pointer items-center rounded-lg border border-gray-200 dark:border-gray-700 p-3 transition hover:border-indigo-400 dark:hover:border-indigo-500 has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50 dark:has-[:checked]:bg-indigo-900/30">
                                            <input type="radio" name="role" value="{{ $role->id }}" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500" @checked(old('role', $user->roles->first()?->id) == $role->id) />
                                            <div class="ms-3">
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-gray-100 bg-gray-50 px-6 py-4 dark:border-gray-700 dark:bg-gray-900/50 sm:px-8">
                        ...
                        <x-primary-button class="bg-indigo-600 hover:bg-indigo-500 focus:ring-indigo-500">{{ __('Update User') }}</x-primary-button>
                    </div>
                </div>
            </form>
        </div>
