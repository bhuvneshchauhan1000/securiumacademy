...
#brichwood .ecu-course {
    position: relative;
    padding-top: 65px; /* space for logos */
}
                            ...
                            @if ($item['category'] === 'universities')
                                ...
                                <div class="course-container">
                                    ...
                                    <div class="course-card">
                                        @if ($u->logo)
                                        <img src="{{ asset($u->logo) }}" alt="{{ $u->name }}" style="max-width:120px; margin-bottom:8px;">
                                        @endif
                                        <p><a href="{{ url('university/' . $u->slug) }}" style="color:#fff; text-decoration:none;">{{ $u->name }}</a></p>
                                        @if ($u->programs->count())
                                        <small style="color:#aaa; display:block;">
                                            @foreach ($u->programs->take(5) as $p)
                                            <a href="{{ url('university/' . $u->slug . '/' . $p->slug) }}" style="color:#8f71ff; display:block; font-size:12px; padding:2px 0;">{{ $p->name }}</a>
                                            @endforeach
                                        </small>
                                        @endif
                                    </div>
                                    @empty
                                    <a href="#" class="course-card"><p>No universities added yet</p></a>
                                    @endforelse
                                </div>
                            @else
                                ...
                                <div class="course-container">
                                    ...
                                    @empty
                                    <a href="#" class="course-card"><p>Courses coming soon</p></a>
                                    @endforelse
                                </div>
                            @endif
                        </div>
            ...
            <div class="collapse navbar-collapse justify-content-center" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="ecitAcademyDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            Course From Securium Academy
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="ecitAcademyDropdown">
                            @foreach ($secMenuItems as $item)
                            <li><a class="dropdown-item" href="{{ url(ltrim($item['url'], '/')) }}">{{ $item['label'] }}</a></li>
                            @endforeach
                        </ul>
                    </li>
                    ...
                    <li class="nav-item dropdown ms-lg-3">
                        <button class="btn btn-light dropdown-toggle" type="button" id="payNowDropdown" data-bs-toggle="dropdown" aria-expanded="false" style="background:#5cc7cc; color:#fff;">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="payNowDropdown">
                            <li><a class="dropdown-item" href="https://pages.razorpay.com/pl_GdJu7xc4YMW3LZ/view" target="_blank">USD</a></li>
                            <li><a class="dropdown-item" href="https://pages.razorpay.com/pl_HhWttIXw6rFLR9/view" target="_blank">INR</a></li>
                        </ul>
                    </li>

                    <li class="nav-item dropdown flag-dropdown ms-lg-3">
                        <button class="btn btn-dark dropdown-toggle d-flex align-items-center" type="button" id="flagDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="flagDropdown">
                            ...
                            <li>
                                <a class="dropdown-item d-flex align-items-center" href="{{ url($flag[2]) }}">
                                    <img src="https://flagcdn.com/w20/{{ $flag[0] }}.png" class="flag-icon me-2"> {{ $flag[1] }}
                                </a>
                            </li>
                            @endforeach
                        </ul>
...
#isaca .isaca-course::after{
    ...
    background-size: 85px;
            ...
            <div class="search-box" id="searchBox">
                <input type="text" class="search-input" placeholder="Search courses..." id="searchInput" autocomplete="off">
                <svg class="search-icon" viewBox="0 0 24 24">
                    <path fill="white" d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0016 9.5 6.5 6.5 0 109.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C8.01 14 6 11.99 6 9.5S8.01 5 10.5 5 15 7.01 15 9.5 12.99 14 10.5 14z"/>
                </svg>
            </div>
